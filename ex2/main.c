...
void *producerWorker(void *arg) {
  ...
  while (true) {
    ...
    // Sinaliza para os consumers que a fila não está mais vazia
...
void *consumerWorker(void *arg) {
  ...
  while (true) {
    ...
    // Sinaliza para os producers que a fila não está mais cheia
...
int main() {
  ...
  // Condição (Semáforo) - Se tiver cheio, não produz
  pthread_cond_init(&not_full, NULL);
  // Condição (Semáforo) - Se tiver vazio, não consome
  pthread_cond_init(&not_empty, NULL);

  pthread_t consumer, producer;

  threadParams workerParams = {.q_buffer = &q_buffer,
                               .mutex = &mutex,
                               .not_full = &not_full,
                               .not_empty = &not_empty};
